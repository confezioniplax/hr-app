...
class QuerySqlHRMYSQL:
    ...
    def list_operator_core_sql(filter_by_department: bool = False) -> str:
        ...
        Restituisce (campi principali per tabella anagrafica):
          id, operator_name, email, phone, fiscal_code, birth_date, hire_date,
          contract_type, contract_expiry, `level`, ral, active, departments
        ...
        return """
            INSERT INTO operators
                (first_name, last_name, fiscal_code, phone, email, address,
                 birth_date, hire_date, contract_type, contract_expiry, `level`, ral, active)
            VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
        """

    @staticmethod
    def update_operator_sql() -> str:
        """Update anagrafica (campi nuovi inclusi)."""
        return """
            UPDATE operators
               SET first_name      = %s,
                   last_name       = %s,
        ...
        return f"""
            SELECT
                ...
                CASE
                    ...
                    WHEN t.requires_expiry = 1 AND oc.expiry_date IS NULL THEN 'ND'
                    WHEN t.requires_expiry = 1 AND oc.expiry_date <  CURRENT_DATE() THEN 'SCADUTA'
                    WHEN t.requires_expiry = 1 AND DATEDIFF(oc.expiry_date, CURRENT_DATE()) <= 30 THEN 'IN_SCADENZA_30'
                    WHEN t.requires_expiry = 1 AND DATEDIFF(oc.expiry_date, CURRENT_DATE()) <= 60 THEN 'IN_SCADENZA_60'
                    WHEN t.requires_expiry = 1 AND DATEDIFF(oc.expiry_date, CURRENT_DATE()) <= 90 THEN 'IN_SCADENZA_90'
                    ELSE 'OK'
                END AS status_calc
            FROM operators o
            {join_dep_filter}
